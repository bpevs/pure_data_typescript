...
class Patch {
  ...
  constructor(
    { records, renderer: renderer1, ...options1 } = DefaultPatchProps,
  ) {
...
function parseChunks(chunks) {
  ...
  chunks.forEach((chunk, index) => {
    ...
    const subPatchIsClosed = isSubPatch &&
      elementType1 === Element1.TYPE.RESTORE;
    ...
    if (
      chunk.recordType === Record.TYPE.ARRAY && prev &&
      Element1.isType(Element1.TYPE.ARRAY, prev)
    ) {
