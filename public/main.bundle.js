...
class PDCoords {
    ...
    toString() {
        return `#X coords ${this.xFrom} ${this.yTo} ${this.xTo} ${this.yFrom}` + ` ${this.width} ${this.height} ${this.graphOnParent ? 1 : 0}`;
    }
}
ctx.lineWidth = "1";
ctx.fillStyle = "black";
...
const getDisplayLength1 = getDisplayLength;
...
function text(xPos1, yPos1, text1, size1) {
    ctx.fillStyle = "black";
    ctx.font = size1 ? `${size1}pt monaco` : "7pt monaco";
...
class PDObj {
    ...
    outlets = [];
    length = 0;
    constructor([xPos1, yPos1, name2, ...params]){
    ...
    render() {
        if (generics[this.name]) {
            this.inlets = generics1[this.name][0];
            this.outlets = generics1[this.name][1];
            this.behavior = generics1[this.name][2];
        }
        this.displayText = this.name.replace(/\\/g, "");
        this.length = getDisplayLength1(this.displayText, this.inlets, this.outlets);
...
class PDObjCnv extends PDObj {
    ...
    constructor(params1){
        super(params1);
        this.width = Number(params1[4]);
        this.height = Number(params1[5]);
        this.label = params1[8] !== "empty" ? this.params[8] : "";
        this.xLabelOffset = Number(params1[9]);
        this.yLabelOffset = Number(params1[10]) + 30;
        this.fontSize = Number(params1[12]) - 8;
        this.bgColor = params1[13];
        this.labelColor = params1[14];
    }
    render() {
        ctx5.fillStyle = parseColor1(this.bgColor);
...
class PDFloatatom {
    ...
    constructor([xPos2, yPos2, width1, lowerLimit, upperLimit, labelPos, label, receive, send]){
    ...
    toString() {
        let str = `#X floatatom ${this.xPos} ${this.yPos} ${this.width}` + ` ${this.lowerLimit} ${this.upperLimit} ${this.labelPos}`;
        str += this.label || "-";
        str += this.receive || "-";
        str += this.send || "-";
        return str;
...
function drawOutline(xPos3, yPos3, length) {
    context.beginPath();
    context.moveTo(xPos3, yPos3);
    context.lineTo(xPos3 + length, yPos3);
    context.lineTo(xPos3 + length + 5, yPos3 + 5);
...
class PDMsg {
    ...
    constructor([xPos3, yPos3, ...params2]){
...
function drawMsgOutline(xPos4, yPos4, length) {
    context.beginPath();
    context.moveTo(xPos4, yPos4);
    context.lineTo(xPos4 + length + 5, yPos4);
    context.lineTo(xPos4 + length, yPos4 + 18 / 4);
    context.lineTo(xPos4 + length, yPos4 + 18 * 3 / 4);
    context.lineTo(xPos4 + length + 5, yPos4 + 18);
    context.lineTo(xPos4, yPos4 + 18);
...
class PDText {
    chunkType = "X";
    elementType = "text";
    constructor([xPos4, yPos4, ...params3]){
        this.xPos = Number(xPos4);
        this.yPos = Number(yPos4);
        this.text = params3.join(" ");
    }
    render() {
        const displayText = this.text.replace(/\\/g, "");
...
const deserializeFromFile1 = deserializeFromFile;
function serializeToFile(elements) {
    return elements.map((el)=>el.chunkType ? el.toString() : [
            "#",
            el.chunk,
            el.element
...
function renderPatch(patch) {
    ...
    patch.forEach((item)=>{
        if (item instanceof PDMsg || item instanceof PDFloatatom || item instanceof PDText || item instanceof PDObj) item.render();
    });
}
...
function listenForHeaderChanges() {
    ...
    modeCheckbox.checked = state1.mode === "edit";
    modeCheckbox.addEventListener("click", function toggleMode() {
        state1.mode = state1.mode === "edit" ? "interactive" : "edit";
    });
...
function listenForCanvasChanges() {
    ...
    patchCanvas.addEventListener("drop", async (e)=>{
        ...
        state1.currentPatch = deserializeFromFile1(patchText);
