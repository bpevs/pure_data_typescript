...
export function deserializeFromFile(text: string) {
  return text
    ...
    .map((line) => {
      ...
      if (subPatchName) {
        const endsGraph = subPatchName === "(subpatch)" &&
          params[2] === "graph";
        const endsSubPatch = element === "restore" &&
          subPatchName === params[3];
      ...
      if (chunk === "X") {
        switch (element) {
          case "array":
            ...
            return prev;
          case "connect":
            return new PDConnect(params);
          case "coords":
            return new PDCoords(params);
          case "floatatom":
            return new PDFloatatom(params);
          case "msg":
            return new PDMsg(params);
          case "text":
            return new PDText(params);
          case "obj": {
